...
- SPMC零拷贝数据流框架
- 基于 C++17 独立实现的多消费者并发同步数据流框架，参考 NVIDIA NvSciStream 架构思想，通过传递静态buffer的指针实现零拷贝、使用位图+水位线无锁同步与内存管理。
- **核心价值**: 
  - 解决了多消费者共享Buffer时的数据竞争问题
  - 通过原子位图替代传统引用计数，实现无锁正确释放packet槽位
  - 预分配环形队列 + 共享只读主Buffer，仅传递指针保证内存数据零拷贝
...
- **Manager**: 流程编排，创建 Channel，注册模块，不碰数据（目前暂由demo代替实现）
...
- 在**多消费者（3 路并行消费）** + 缓冲池（pool_size=5） 下，虚拟生产者生成 30 帧数据，并完成全链路传递至所有消费者消费输出，**总耗时 20 毫秒**，整体数据传输延迟极低，满足实时性要求。
